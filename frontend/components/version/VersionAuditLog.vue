      ...
      <div class="flex flex-wrap gap-4 items-center">
        <div class="flex items-center gap-2">
          ...
          >
            ...
            <option value="CREATE_VERSION">สร้างเวอร์ชัน</option>
            <option value="UPDATE_NETWORK">แผนภาพท่อ</option>
            <option value="UPDATE_FIXTURES">สุภัณฑ์</option>
            <option value="CALCULATE">คำนวณ</option>
            <option value="APPLY_PIPE_SIZE">ปรับขนาดท่อ</option>
            <option value="UPLOAD_BLUEPRINT">Blueprint</option>
...
const getActionTitle = (action: string): string => {
  const titles: Record<string, string> = {
    ...
    UPDATE_NETWORK: 'แก้ไขแผนภาพท่อ',
    UPDATE_FIXTURES: 'แก้ไขสุขภัณฑ์',
    CALCULATE: 'คำนวณระบบท่อ',
    APPLY_PIPE_SIZE: 'ปรับขนาดท่อ',
    UPLOAD_BLUEPRINT: 'อัปโหลด Blueprint',
    DUPLICATE: 'คัดลอกเวอร์ชัน',
    RESTORE: 'คืนค่าเวอร์ชัน',
    UPDATE: 'แก้ไข',
  }
...
const getActionBadgeClass = (action: string): string => {
  const classes: Record<string, string> = {
    ...
    UPLOAD_BLUEPRINT: 'bg-indigo-100 text-indigo-800',
...
const getChangesSummary = (log: AuditLog): string => {
  if (log.details) {
    try {
      ...
      if (typeof details === 'object') {
        // ให้เช็คประเภท Action ควบคู่ไปด้วย เพื่อความถูกต้องของการแสดงผล
        if (log.action === 'UPDATE_FIXTURES' && details.snapshotFixtures) {
          const fixtures = typeof details.snapshotFixtures === 'string' 
            ? JSON.parse(details.snapshotFixtures) 
            : details.snapshotFixtures
          const fixtureCount = fixtures.nodes?.reduce((sum: number, node: any) =>
            sum + (node.fixtures?.length || 0), 0) || 0
          return `แก้ไข ${fixtureCount} สุขภัณฑ์`
        }

        if (log.action === 'UPDATE_NETWORK' && details.snapshotNetwork) {
          const network = typeof details.snapshotNetwork === 'string' 
            ? JSON.parse(details.snapshotNetwork) 
            : details.snapshotNetwork
  ...
  return `${log.entity || 'version'} ${log.action.toLowerCase()}`
}
