      ...
      <div class="px-4 py-6 sm:px-0">
          ...
          <div class="flex items-center gap-4 mb-4">
            <div>
              ...
              <p class="mt-1 text-sm text-gray-600">
                เลือก 2 Version เพื่อเปรียบเทียบ Network, Fixtures และ
                Calculation
              </p>
            </div>
          </div>
        ...
        <div class="bg-white rounded-lg shadow-sm p-6 mb-6">
          <h3 class="text-lg font-medium text-gray-900 mb-4">
            เลือก Version ที่ต้องการเปรียบเทียบ
          </h3>
        ...
        <div v-if="comparisonResult" class="space-y-6">
            ...
            <div class="bg-white rounded-lg shadow-sm p-6">
              ...
              <div class="space-y-2 text-sm">
                <div class="flex justify-between">
                  <span class="text-gray-600">Version A:</span>
                  <span class="font-medium"
                    >{{ countFixtures(versionAData) }} fixtures</span
                  >
                </div>
                <div class="flex justify-between">
                  <span class="text-gray-600">Version B:</span>
                  <span class="font-medium"
                    >{{ countFixtures(versionBData) }} fixtures</span
                  >
                </div>
                <div class="flex justify-between border-t pt-2">
                  <span class="text-gray-600">Difference:</span>
                  <span
                    ...
                    :class="
                      getDiffClass(
                        countFixtures(versionBData),
                        countFixtures(versionAData)
                      )
                    "
                  >
                    {{
          ...
          <div class="bg-white rounded-lg shadow-sm p-6">
            ...
            <div class="grid grid-cols-1 gap-6">
              ...
              <!-- Version B Network -->
              <div
                class="border-2 border-orange-200 rounded-lg overflow-hidden"
              >
            ...
            <div class="space-y-4">
              ...
              <div v-if="versionA" class="border-l-4 border-blue-500 pl-4">
                ...
                <div class="text-sm text-gray-600 space-y-1">
                  ...
                  <p>
                    • อัปเดตล่าสุด: {{ formatDateTime(versionA.updatedAt) }}
                  </p>
                  <p v-if="versionA.createdBy">
                    • สร้างโดย: {{ versionA.createdBy }}
                  </p>
                  <p v-if="!versionA.snapshotNetwork">• ยังไม่มี Network</p>
                  <p v-else>
                    • มี Network ({{ getNetworkStats(versionA).nodes }} nodes,
                    {{ getNetworkStats(versionA).pipes }} pipes)
                  </p>
                  <p v-if="!versionA.snapshotFixtures">• ยังไม่มี Fixtures</p>
                  <p v-else>
                    • มี Fixtures ({{
                      countFixtures(JSON.parse(versionA.snapshotFixtures))
                    }}
                    fixtures)
                  </p>
              ...
              <div v-if="versionB" class="border-l-4 border-orange-500 pl-4">
                ...
                <div class="text-sm text-gray-600 space-y-1">
                  ...
                  <p>
                    • อัปเดตล่าสุด: {{ formatDateTime(versionB.updatedAt) }}
                  </p>
                  <p v-if="versionB.createdBy">
                    • สร้างโดย: {{ versionB.createdBy }}
                  </p>
                  <p v-if="!versionB.snapshotNetwork">• ยังไม่มี Network</p>
                  <p v-else>
                    • มี Network ({{ getNetworkStats(versionB).nodes }} nodes,
                    {{ getNetworkStats(versionB).pipes }} pipes)
                  </p>
                  <p v-if="!versionB.snapshotFixtures">• ยังไม่มี Fixtures</p>
                  <p v-else>
                    • มี Fixtures ({{
                      countFixtures(JSON.parse(versionB.snapshotFixtures))
                    }}
                    fixtures)
                  </p>
        ...
        <div v-else class="bg-white rounded-lg shadow-sm p-12 text-center">
          ...
          <p class="text-gray-600">
            เลือก 2 Version ที่ต้องการเปรียบเทียบ Network, Fixtures และ
            Calculation Results
          </p>
        </div>
...
const parseVersionData = (version: any) => {
  ...
  console.log(
    "[parseVersionData] Has snapshotNetwork:",
    !!version.snapshotNetwork
  );

  try {
    const network = version.snapshotNetwork
      ? JSON.parse(version.snapshotNetwork)
      : null;

    const data = {
      snapshotNetwork: network,
      snapshotFixtures: version.snapshotFixtures
        ? JSON.parse(version.snapshotFixtures)
        : null,
      snapshotResults: version.snapshotResults
        ? JSON.parse(version.snapshotResults)
        : null
    };
...
const loadBlueprintsForVersion = async (
  versionId: string | number,
  versionLabel: "A" | "B"
) => {
  console.log(
    `[loadBlueprintsForVersion] Loading for Version ${versionLabel}, ID: ${versionId}`
  );
  ...
  try {
    ...
    console.log(
      `✅ Loaded blueprints for Version ${versionLabel}:`,
      blueprints?.length || 0
    );
  } catch (error) {
    console.error(
      `Failed to load blueprints for Version ${versionLabel}:`,
      error
    );
    if (versionLabel === "A") {
