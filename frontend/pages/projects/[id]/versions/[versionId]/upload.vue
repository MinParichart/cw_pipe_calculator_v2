              ...
              <div class="space-y-4">
                ...
                <div class="bg-blue-50 border border-blue-200 rounded-lg p-4">
                  <h4 class="text-sm font-semibold text-blue-900 mb-3">
                    <span
                      class="inline-flex items-center justify-center w-5 h-5 rounded-full bg-blue-600 text-white text-xs mr-2"
                      >1</span
                    >
                    ระบุตำแหน่ง
                  </h4>
                  <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <p class="text-xs text-gray-500 mt-1">
                        เลือกชั้นที่ต้องการอัปโหลดแปลน
                      </p>
                    </div>
                    ...
                    <div class="flex-1">
                      ...
                      <p class="text-xs text-yellow-700 mt-1">
                        {{ existingBlueprint.name }} ({{
                          existingBlueprint.typeText
                        }}) - อัปโหลดเมื่อ
                        {{ formatDate(existingBlueprint.createdAt) }}
                      </p>
                      ...
                      <div class="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p class="text-xs text-gray-500">
                            {{ formatFileSize(blueprintFile.size) }}
                          </p>
                        </div>
                      </div>
                      <button
                ...
                <!-- Step 3: Notes -->
                <div
                  v-if="blueprintFile && !existingBlueprint"
                  class="space-y-4"
                >
                  <h4 class="text-sm font-semibold text-gray-700">
                    <span
            ...
            <div class="p-4">
              ...
              <div class="mt-4 grid grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  {{ previewingBlueprint.floorText }}
                </div>
                <div>
                  <span class="font-medium">ประเภท:</span>
                  {{ previewingBlueprint.typeText }}
                </div>
                <div>
                  <span class="font-medium">สเกล:</span>
                  {{ previewingBlueprint.scale }}
                </div>
                <div>
                  <span class="font-medium">อัปโหลด:</span>
                  {{ formatDate(previewingBlueprint.createdAt) }}
                </div>
              </div>
              <div
                v-if="previewingBlueprint.notes"
                class="mt-3 p-3 bg-gray-50 rounded"
              >
                <span class="font-medium">หมายเหตุ:</span>
                {{ previewingBlueprint.notes }}
              </div>
            </div>
          </div>
        </div>
...
const existingBlueprint = computed(() => {
  ...
  return (
    blueprints.value.find(
      (bp) =>
        bp.floor === blueprintFloor.value && bp.type === blueprintType.value
    ) || null
  );
});
...
const loadVersion = async () => {
  ...
  try {
    ...
    const result = await versionStore.loadVersions(
...
const handleFileUpload = (event: Event) => {
  ...
  if (target.files && target.files[0]) {
    ...
    if (!validTypes.includes(file.type)) {
      toast.error(
        "Blueprint รองรับเฉพาะไฟล์รูปภาพ (PNG, JPG) เท่านั้น\nกรุณาแปลง PDF เป็นรูปภาพก่อนอัปโหลด"
      );
      return;
    }
  ...
  try {
    ...
    const uploadedBlueprint = await documentsApi.uploadForVersion(
      ...
      blueprintFile.value,
      {
        ...
        scale: "" // Scale will be set in network builder
      }
    );
...
const saveBlueprintEdit = () => {
  ...
  const blueprintIndex = blueprints.value.findIndex(
    (bp) => bp.id === editingBlueprint.value.id
  );
  if (blueprintIndex === -1) {
  ...
  blueprints.value[blueprintIndex] = {
    ...
    scale: bp.scale || "", // Keep existing scale (will be set in network builder)
