...
export const HUNTER_CURVE_FLUSH_TANK = [
  { fu: 6, gpm: 5.0 },
  { fu: 8, gpm: 6.5 },
  { fu: 10, gpm: 8.0 },
  { fu: 12, gpm: 9.2 },
...
export const HUNTER_CURVE_FLUSH_VALVE = [
  { fu: 10, gpm: 27.0 },
  { fu: 12, gpm: 28.6 },
  { fu: 14, gpm: 30.2 },
  { fu: 16, gpm: 31.8 },
...
export function getFlowRate(fixtureUnits: number, systemType: 'FLUSH_TANK' | 'FLUSH_VALVE' = 'FLUSH_TANK'): number {
  return fuToGPM(fixtureUnits, systemType)
}

// Validate FU is within range
export function isValidFU(fu: number, systemType: 'FLUSH_TANK' | 'FLUSH_VALVE' = 'FLUSH_TANK'): boolean {
  const table = systemType === 'FLUSH_TANK' ? HUNTER_CURVE_FLUSH_TANK : HUNTER_CURVE_FLUSH_VALVE
  return fu >= 0 && fu <= table[table.length - 1].fu
}

// Get FU range for system type
export function getFURange(systemType: 'FLUSH_TANK' | 'FLUSH_VALVE' = 'FLUSH_TANK'): { min: number; max: number } {
  const table = systemType === 'FLUSH_TANK' ? HUNTER_CURVE_FLUSH_TANK : HUNTER_CURVE_FLUSH_VALVE
  return {
    min: table[0].fu,
