    ...
    <div class="px-6 py-3">
      <div class="max-w-[1400px] mx-auto">
        <div class="relative flex items-center justify-between gap-1">
          ...
          <div
            class="absolute top-[22px] left-11 right-11 h-0.5 bg-slate-200"
            style="z-index: 0"
          ></div>

          <!-- Progress Line Fill -->
          <div
            class="absolute top-[22px] left-11 h-0.5 bg-green-500 transition-all duration-500"
            style="z-index: 0"
          ...
          >
            ...
            <div class="flex flex-col items-center">
              <button
                ...
                :disabled="!versionId"
                class="w-11 h-11 rounded-full flex items-center justify-center font-semibold text-sm transition-all cursor-pointer hover:scale-110 disabled:opacity-50 disabled:cursor-not-allowed"
                :class="{
                  'bg-green-500 text-white shadow-md hover:shadow-lg':
                    isCompleted(step.id),
                  'bg-blue-600 text-white shadow-md ring-4 ring-blue-100 hover:shadow-lg':
                    isCurrent(step.id),
              ...
              >
                ...
                <span v-else class="text-sm">{{ step.order }}</span>
              </button>
...
const projectDisplayName = computed(() => {
  console.log(
    "[VersionSteps] projectDisplayName computed, projectData:",
    projectData.value
  );
...
const currentVersionName = computed(() => {
  console.log(
    "[VersionSteps] currentVersionName computed, versionId:",
    props.versionId
  );
  if (props.versionId && versionStore.versions) {
    const version = versionStore.versions.find(
      (v: any) => v.id === parseInt(props.versionId as string)
    );
...
const getProgressWidth = () => {
  ...
  const completedSteps = VERSION_STEPS.filter((step) =>
    isCompleted(step.id)
  ).length;
...
onMounted(async () => {
  try {
    ...
    if (projectId) {
      projectData.value = await useApi().projectsApi.get(
