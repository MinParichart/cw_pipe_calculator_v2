      ...
      <div class="px-4 py-6 sm:px-0">
        <div class="mb-6">
          ...
          <p class="text-sm text-gray-600">
            อัปโหลดเอกสาร แบบแปลนสำหรับการวางแปลนท่อน้ำดี
          </p>
        </div>
              ...
              <div class="space-y-4">
                ...
                <div v-if="blueprintFile && !existingBlueprint" class="space-y-4">
                  ...
                  <div class="mt-2 text-xs bg-gray-100 p-2 rounded">
                    <div>File: {{ blueprintFile?.name || 'None' }}</div>
                    <div>Floor: {{ blueprintFloor || 'None' }}</div>
                    <div>Scale: {{ blueprintScale || 'None' }}</div>
                    <div>Disabled: {{ !blueprintFile || !blueprintFloor || !blueprintScale || saving }}</div>
                  </div>
                </div>
              </div>
            </div>
...
const saveBlueprint = async () => {
  console.log('🔘 saveBlueprint called')
  console.log('📁 File:', blueprintFile.value?.name)
  console.log('🏢 Floor:', blueprintFloor.value)
  console.log('📏 Scale:', blueprintScale.value)
  ...
  try {
    ...
    console.log('📤 Calling documentsApi.upload...')
    const uploadedBlueprint = await documentsApi.upload(
      projectId,
      blueprintFile.value,
      {
    ...
    console.log('✅ Upload success:', uploadedBlueprint)
    ...
    workflowStore.markStepComplete('documents')
  } catch (error: any) {
...
const loadFromLocalStorage = () => {
  ...
  const key = `blueprints_${projectId}`
...
onMounted(async () => {
  workflowStore.setCurrentStep('documents')
  await loadFromAPI()
})
